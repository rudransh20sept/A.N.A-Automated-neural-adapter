...
# Updated with QLoRA (4-bit), FP16 controls, and Gradient Accumulation.
...
from transformers import (
    ...
    BitsAndBytesConfig # NEW: Required for 4-bit loading
)
...
def train_and_merge(
    ...
    # NEW: Configure 4-bit Quantization (QLoRA)
    bnb_config = None
    if use_4bit:
        logger_callback("🔧 Configuring 4-bit quantization (QLoRA) for memory efficiency...")
        bnb_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_use_double_quant=True,
            bnb_4bit_quant_type="nf4",
            # IMPORTANT: For GTX 1080 (Pascal), compute_dtype must be float32 to avoid unscale errors
            bnb_4bit_compute_dtype=torch.float32 
        )
        model_load_kwargs["quantization_config"] = bnb_config
    else:
        # If not using 4-bit, load in float16 for standard LoRA if device supports it, otherwise float32
        model_load_kwargs["torch_dtype"] = torch.float16 if use_fp16 else torch.float32
    ...
    # Pre-processing for 4-bit training
    if use_4bit:
    ...
    training_args = TrainingArguments(
        ...
        gradient_accumulation_steps=gradient_accumulation_steps, # NEW
        ...
        report_to="none",
        seed=42,
        fp16=use_fp16, # Controlled by GUI now
    ...
    logger_callback(f" Batch size: {batch_size}, Grad Accum: {gradient_accumulation_steps}")
    ...
    # Note: We usually merge in FP16 or FP32, rarely in 4-bit (merging 4-bit is lossy/complex)
    # We will reload in FP16 if possible for the merge, or FP32.
    merge_kwargs = {
        "low_cpu_mem_usage": True,
    ...
    if offload_folder:
        merge_kwargs["offload_folder"] = offload_folder

    base_model = AutoModelForCausalLM.from_pretrained(model_id, **merge_kwargs)
    base_tokenizer = AutoTokenizer.from_pretrained(model_id, **auth_args)
    
    logger_callback(" Loading adapter...")
    lora_model = PeftModel.from_pretrained(base_model, lora_adapter_output_dir)
    
    logger_callback(" Running merge_and_unload...")
...
class LoRATrainerApp:
    def __init__(self, root: tk.Tk):
        ...
        self.root.geometry("1100x850") # Slightly larger
        ...
        self.use_4bit = tk.BooleanVar(value=True) # Default True for large models
        self.use_fp16 = tk.BooleanVar(value=False) # Default False for GTX 1080 stability
        self.grad_accum_steps = tk.IntVar(value=4) # Default 4 to simulate batch size 4
        # ---------------------
    ...
    def create_widgets(self):
        ...
        self._create_hardware_section(left_frame) # NEW SECTION
        ...
        self.console_output = tk.Text(
            console_frame, wrap=tk.WORD, height=30, state=tk.DISABLED, 
            relief=tk.FLAT, bg="#1b263b", fg="#e0e1dd", 
    ...
    def _create_model_output_section(self, parent):
        ...
        self._create_input_row(frame, 1, "Hugging Face Token:", self.hf_token, show='*')
    ...
    # --- NEW SECTION: Hardware & Advanced ---
    def _create_hardware_section(self, parent):
        frame = ttk.LabelFrame(parent, text="Hardware & Advanced Settings", padding=15)
        frame.pack(fill=tk.X, pady=5, padx=10)
        
        # Row 0: 4-bit Quantization (QLoRA)
        ttk.Checkbutton(frame, text="Enable 4-bit Quantization (QLoRA) - Train 7B+ models on 8GB VRAM", 
                        variable=self.use_4bit).grid(row=0, column=0, columnspan=2, sticky="w", padx=5, pady=5)
        
        # Row 1: FP16 (Warning label included)
        fp16_check = ttk.Checkbutton(frame, text="Enable FP16 Training (Faster)", variable=self.use_fp16)
        fp16_check.grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ttk.Label(frame, text="(Disable if you get 'unscale' errors on GTX 10 series)", font=("Segoe UI", 8)).grid(row=1, column=1, sticky="w")
        
        # Row 2: Disk Offload
        offload_check = ttk.Checkbutton(frame, text="Enable Disk Offload", variable=self.enable_disk_offload, command=self._toggle_offload_widgets)
        offload_check.grid(row=2, column=0, sticky="w", padx=5, pady=5)
        self.offload_widgets = self._create_browse_row(frame, 3, "Offload Folder:", self.offload_folder_path, self.select_offload_directory)
        self._toggle_offload_widgets()
    ...
    def _create_lora_section(self, parent):
        ...
        self._create_input_row(frame, 3, "Target Modules (csv):", self.target_modules)
    ...
    def _create_training_section(self, parent):
        ...
        self._create_input_row(frame, 1, "Max Seq Length:", self.max_length)
        self._create_input_row(frame, 2, "Batch Size (Per Device):", self.batch_size)
        
        # NEW: Gradient Accumulation
        self._create_input_row(frame, 3, "Gradient Accum. Steps:", self.grad_accum_steps)
        ttk.Label(frame, text="(Increase this if you decrease Batch Size)", font=("Segoe UI", 8)).grid(row=4, column=1, columnspan=2, sticky="w", padx=5)

        self._create_input_row(frame, 5, "Learning Rate:", self.learning_rate)
        
        optimizer_check = ttk.Checkbutton(frame, text="Use Paged Optimizer (Saves VRAM)", variable=self.use_paged_optimizer)
        optimizer_check.grid(row=6, column=0, columnspan=3, sticky="w", padx=5, pady=8)
    ...
    def _toggle_offload_widgets(self):
        if self.enable_disk_offload.get():
            for widget in self.offload_widgets: widget.grid()
        else:
            for widget in self.offload_widgets: widget.grid_remove()

    def _toggle_dataset_fields(self):
        source = self.dataset_source.get()
        if source == "local":
            for widget in self.local_widgets: widget.grid()
            for widget in self.hf_id_widgets: widget.grid_remove()
            for widget in self.hf_split_widgets: widget.grid_remove()
        else:
            for widget in self.local_widgets: widget.grid_remove()
            for widget in self.hf_id_widgets: widget.grid()
            for widget in self.hf_split_widgets: widget.grid()

    def open_file_dialog(self):
        file_path = filedialog.askopenfilename(title="Select Dataset File", filetypes=[("Text files", "*.txt"), ("All files", "*.*")])
        if file_path: self.local_dataset_path.set(file_path)
    ...
    def select_offload_directory(self):
        ...
        if folder: self.offload_folder_path.set(folder)
    ...
    def _run_training(self):
        ...
        try:
            ...
            if self.enable_disk_offload.get():
                ...
                if not path: raise ValueError("Disk Offload is enabled, but no offload folder was selected.")
                offload_path = path
            ...
            train_params = {
                ...
                # --- NEW PARAMS PASSED ---
                "use_4bit": self.use_4bit.get(),
                "use_fp16": self.use_fp16.get(),
                "gradient_accumulation_steps": self.grad_accum_steps.get(),
                # -------------------------
                "logger_callback": print,
            }
